...
author_label = tk.Label(root,text = 'Author Details:Dhairya Maheshwari',font=('Arial',14,"bold italic") ,bg="black",foreground="white").pack()
...
ctLabel = tk.Label(root,text = 'Measure of Central Tendancy',font=('Arial',12),bg=
                   "black",foreground="white" ).place(x=10, y=150)
...
b4=tk.Radiobutton(root,text="Quartile Deviation",command=lambda:qd(),bg="black",foreground="white").place(x=10,y=360)
b5=tk.Radiobutton(root,text="Mean Deviation",command=lambda:md(),bg="black",foreground="white").place(x=10,y=380)
...
def qd():
    l1=list(map(int,b_l1.get().split(",")))
    q1=float(np.quantile(l1,0.25))
    q3=float(np.quantile(l1,0.75))   
    q_d=float((q3-q1)/2)
    st1="Quartile Deviation is: "+str(q_d)
    lb2.config(text=st1)
...
def md():
    ...
    for i in l1:
        r.append(abs(i-m))
    st1="Mean Deviation is: "+str(st.mean(r))
    lb2.config(text=st1)    
